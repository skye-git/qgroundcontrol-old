...
#define MAVLINK_MESSAGE_INFO_BATTERY_VOLTAGE { \
	...
	{  { "voltage_1", NULL, MAVLINK_TYPE_UINT16_T, 0, 0, offsetof(mavlink_battery_voltage_t, voltage_1) }, \
         ...
         { "accu_id", NULL, MAVLINK_TYPE_UINT8_T, 0, 8, offsetof(mavlink_battery_voltage_t, accu_id) }, \
...
static inline uint16_t mavlink_msg_battery_voltage_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
						       uint8_t accu_id, uint16_t voltage_1, uint16_t voltage_2, uint16_t voltage_3, uint16_t voltage_4, int8_t battery_remaining)
{
	...
	_mav_put_uint8_t(buf, 8, accu_id);
	...
	return mavlink_finalize_message(msg, system_id, component_id, 10, 135);
}
						           ...
						           uint8_t accu_id,uint16_t voltage_1,uint16_t voltage_2,uint16_t voltage_3,uint16_t voltage_4,int8_t battery_remaining)
{
	...
	msg->msgid = MAVLINK_MSG_ID_BATTERY_VOLTAGE;
	return mavlink_finalize_message_chan(msg, system_id, component_id, chan, 10, 135);
}
...
{
	return mavlink_msg_battery_voltage_pack(system_id, component_id, msg, battery_voltage->accu_id, battery_voltage->voltage_1, battery_voltage->voltage_2, battery_voltage->voltage_3, battery_voltage->voltage_4, battery_voltage->battery_remaining);
}
...
static inline void mavlink_msg_battery_voltage_send(mavlink_channel_t chan, uint8_t accu_id, uint16_t voltage_1, uint16_t voltage_2, uint16_t voltage_3, uint16_t voltage_4, int8_t battery_remaining)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
	char buf[10];
	_mav_put_uint16_t(buf, 0, voltage_1);
	...
	_mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_BATTERY_VOLTAGE, buf, 10, 135);
#else
	mavlink_battery_voltage_t packet;
	packet.voltage_1 = voltage_1;
	packet.voltage_2 = voltage_2;
	...
	_mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_BATTERY_VOLTAGE, (const char *)&packet, 10, 135);
#endif
}
...
/**
 * @brief Get field accu_id from battery_voltage message
 *
 * @return Accupack ID
 */
static inline uint8_t mavlink_msg_battery_voltage_get_accu_id(const mavlink_message_t* msg)
{
...
static inline void mavlink_msg_battery_voltage_decode(const mavlink_message_t* msg, mavlink_battery_voltage_t* battery_voltage)
{
	...
	battery_voltage->accu_id = mavlink_msg_battery_voltage_get_accu_id(msg);
