...
#if MAVLINK_MSG_ID_POSITION_CONTROLLER_RAW_LEN <= MAVLINK_MAX_PAYLOAD_LEN
/*
 ...
 */
static inline void mavlink_msg_position_controller_raw_send_buf(mavlink_message_t *msgbuf, mavlink_channel_t chan,  float F_x, float F_y, float F_z)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
...
#endif
#else
	mavlink_position_controller_raw_t *packet = (mavlink_position_controller_raw_t *)msgbuf;
	packet->F_x = F_x;
	packet->F_y = F_y;
	packet->F_z = F_z;
