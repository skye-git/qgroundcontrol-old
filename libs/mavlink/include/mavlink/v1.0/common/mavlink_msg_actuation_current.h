...
typedef struct __mavlink_actuation_current_t
{
 ...
 uint8_t component_id; ///< ID number
...
#define MAVLINK_MESSAGE_INFO_ACTUATION_CURRENT { \
	...
	{  { "current_1", NULL, MAVLINK_TYPE_INT16_T, 0, 0, offsetof(mavlink_actuation_current_t, current_1) }, \
         ...
         { "component_id", NULL, MAVLINK_TYPE_UINT8_T, 0, 8, offsetof(mavlink_actuation_current_t, component_id) }, \
         } \
}
...
static inline uint16_t mavlink_msg_actuation_current_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
						       uint8_t component_id, int16_t current_1, int16_t current_2, int16_t current_3, int16_t current_4)
{
	...
	_mav_put_uint8_t(buf, 8, component_id);
	...
	return mavlink_finalize_message(msg, system_id, component_id, 9, 66);
}
						           ...
						           uint8_t component_id,int16_t current_1,int16_t current_2,int16_t current_3,int16_t current_4)
{
	...
	msg->msgid = MAVLINK_MSG_ID_ACTUATION_CURRENT;
	return mavlink_finalize_message_chan(msg, system_id, component_id, chan, 9, 66);
}
...
{
	return mavlink_msg_actuation_current_pack(system_id, component_id, msg, actuation_current->component_id, actuation_current->current_1, actuation_current->current_2, actuation_current->current_3, actuation_current->current_4);
}
...
static inline void mavlink_msg_actuation_current_send(mavlink_channel_t chan, uint8_t component_id, int16_t current_1, int16_t current_2, int16_t current_3, int16_t current_4)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
	char buf[9];
	_mav_put_int16_t(buf, 0, current_1);
	...
	_mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_ACTUATION_CURRENT, buf, 9, 66);
#else
	mavlink_actuation_current_t packet;
	packet.current_1 = current_1;
	packet.current_2 = current_2;
	...
	_mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_ACTUATION_CURRENT, (const char *)&packet, 9, 66);
#endif
}
...
/**
 * @brief Get field component_id from actuation_current message
 *
 * @return ID number
 */
static inline uint8_t mavlink_msg_actuation_current_get_component_id(const mavlink_message_t* msg)
{
...
static inline void mavlink_msg_actuation_current_decode(const mavlink_message_t* msg, mavlink_actuation_current_t* actuation_current)
{
	...
	actuation_current->component_id = mavlink_msg_actuation_current_get_component_id(msg);
#else
