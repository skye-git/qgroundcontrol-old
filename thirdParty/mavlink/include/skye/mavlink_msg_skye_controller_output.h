...
typedef struct __mavlink_skye_controller_output_t
{
 ...
 uint16_t force_x; ///< Signal given from controller to actuation chain FIXME: VALUES
 uint16_t force_y; ///< Signal given from controller to actuation chain FIXME: VALUES
 uint16_t force_z; ///< Signal given from controller to actuation chain FIXME: VALUES
 uint16_t moment_x; ///< Signal given from controller to actuation chain FIXME: VALUES
 uint16_t moment_y; ///< Signal given from controller to actuation chain FIXME: VALUES
 uint16_t moment_z; ///< Signal given from controller to actuation chain FIXME: VALUES
...
#define MAVLINK_MESSAGE_INFO_SKYE_CONTROLLER_OUTPUT { \
	...
	{  { "time_usec", NULL, MAVLINK_TYPE_UINT32_T, 0, 0, offsetof(mavlink_skye_controller_output_t, time_usec) }, \
         { "force_x", NULL, MAVLINK_TYPE_UINT16_T, 0, 4, offsetof(mavlink_skye_controller_output_t, force_x) }, \
         { "force_y", NULL, MAVLINK_TYPE_UINT16_T, 0, 6, offsetof(mavlink_skye_controller_output_t, force_y) }, \
         { "force_z", NULL, MAVLINK_TYPE_UINT16_T, 0, 8, offsetof(mavlink_skye_controller_output_t, force_z) }, \
         { "moment_x", NULL, MAVLINK_TYPE_UINT16_T, 0, 10, offsetof(mavlink_skye_controller_output_t, moment_x) }, \
         { "moment_y", NULL, MAVLINK_TYPE_UINT16_T, 0, 12, offsetof(mavlink_skye_controller_output_t, moment_y) }, \
         { "moment_z", NULL, MAVLINK_TYPE_UINT16_T, 0, 14, offsetof(mavlink_skye_controller_output_t, moment_z) }, \
         } \
}
 ...
 * @param force_x Signal given from controller to actuation chain FIXME: VALUES
 * @param force_y Signal given from controller to actuation chain FIXME: VALUES
 * @param force_z Signal given from controller to actuation chain FIXME: VALUES
 * @param moment_x Signal given from controller to actuation chain FIXME: VALUES
 * @param moment_y Signal given from controller to actuation chain FIXME: VALUES
 * @param moment_z Signal given from controller to actuation chain FIXME: VALUES
...
static inline uint16_t mavlink_msg_skye_controller_output_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
						       uint32_t time_usec, uint16_t force_x, uint16_t force_y, uint16_t force_z, uint16_t moment_x, uint16_t moment_y, uint16_t moment_z)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
	char buf[16];
	_mav_put_uint32_t(buf, 0, time_usec);
	_mav_put_uint16_t(buf, 4, force_x);
	_mav_put_uint16_t(buf, 6, force_y);
	_mav_put_uint16_t(buf, 8, force_z);
	_mav_put_uint16_t(buf, 10, moment_x);
	_mav_put_uint16_t(buf, 12, moment_y);
	_mav_put_uint16_t(buf, 14, moment_z);
	...
	return mavlink_finalize_message(msg, system_id, component_id, 16, 82);
}
						           ...
						           uint32_t time_usec,uint16_t force_x,uint16_t force_y,uint16_t force_z,uint16_t moment_x,uint16_t moment_y,uint16_t moment_z)
{
	...
	msg->msgid = MAVLINK_MSG_ID_SKYE_CONTROLLER_OUTPUT;
	return mavlink_finalize_message_chan(msg, system_id, component_id, chan, 16, 82);
}
...
static inline void mavlink_msg_skye_controller_output_send(mavlink_channel_t chan, uint32_t time_usec, uint16_t force_x, uint16_t force_y, uint16_t force_z, uint16_t moment_x, uint16_t moment_y, uint16_t moment_z)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
	char buf[16];
	_mav_put_uint32_t(buf, 0, time_usec);
	...
	_mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_SKYE_CONTROLLER_OUTPUT, buf, 16, 82);
#else
	mavlink_skye_controller_output_t packet;
	packet.time_usec = time_usec;
	packet.force_x = force_x;
	...
	_mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_SKYE_CONTROLLER_OUTPUT, (const char *)&packet, 16, 82);
#endif
}
 ...
 *
 * @return Signal given from controller to actuation chain FIXME: VALUES
 */
static inline uint16_t mavlink_msg_skye_controller_output_get_force_x(const mavlink_message_t* msg)
{
 ...
 * @return Signal given from controller to actuation chain FIXME: VALUES
 */
static inline uint16_t mavlink_msg_skye_controller_output_get_force_y(const mavlink_message_t* msg)
{
 ...
 * @return Signal given from controller to actuation chain FIXME: VALUES
 */
static inline uint16_t mavlink_msg_skye_controller_output_get_force_z(const mavlink_message_t* msg)
{
 ...
 * @return Signal given from controller to actuation chain FIXME: VALUES
 */
static inline uint16_t mavlink_msg_skye_controller_output_get_moment_x(const mavlink_message_t* msg)
{
 ...
 * @return Signal given from controller to actuation chain FIXME: VALUES
 */
static inline uint16_t mavlink_msg_skye_controller_output_get_moment_y(const mavlink_message_t* msg)
{
 ...
 * @return Signal given from controller to actuation chain FIXME: VALUES
 */
static inline uint16_t mavlink_msg_skye_controller_output_get_moment_z(const mavlink_message_t* msg)
{
